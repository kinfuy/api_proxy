...
const createHooks = () => {
  ...
  const beforeXmlRequest = (proxyContent: ProxyContent) => {
...
const initProxy = (
  ...
  if (webSite.isProxy) {
    const SELF_XML_HTTP_REQUEST = initXMLHttpRequest(
      originXMLHttpRequest,
      apiProxy,
      beforeXmlOpen,
      beforeXmlRequest,
      beforeXmlResponse
    );
    const SELF_FETCH = initFetchProxy(
      originFetch,
      apiProxy
    ) as typeof originFetch;
...
addEventListener(window, 'message', (info: any) => {
  ...
  switch (message.key) {
    case EVENT_KEY.API_PROXY_BACKGROUND_UPDATE: {
      ...
      // 监听到背景页通知需要更新代理信息
      // 重新初始化xml fetch
      const { beforeXmlOpen, beforeXmlRequest, beforeXmlResponse } =
        createHooks();
      initProxy(
        message.data.webSite,
        message.data.apiProxy,
        beforeXmlOpen,
        beforeXmlRequest,
        beforeXmlResponse
      );
